...
import {
  ...
  TrendingUp, MapPin, Bell, RefreshCw, Layers, Zap, UserCheck, ChevronRight
} from 'lucide-react'
...
export default function DashboardPage() {
    ...
    return (
      <div className="min-h-screen bg-[#080811] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <Card className="max-w-md w-full bg-slate-950/60 border border-slate-800/80 p-8 backdrop-blur-2xl rounded-2xl relative shadow-2xl shadow-slate-950/50">
          ...
          {/* Tab switches for login mode */}
          <div className="grid grid-cols-2 gap-2 bg-slate-900/50 border border-slate-800/60 p-1 rounded-xl mb-4">
            <button
              onClick={() => {
                setAdminLoginMode('password')
                setAdminError('')
              }}
              className={`py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors ${adminLoginMode === 'password' ? 'bg-primary text-zinc-950' : 'text-zinc-400 hover:text-zinc-200'
                }`}
            >
              Password Auth
            </button>
            <button
              ...
              className={`py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors ${adminLoginMode === 'otp' ? 'bg-primary text-zinc-950' : 'text-zinc-400 hover:text-zinc-200'
                }`}
            >
              Phone OTP
            </button>
              ...
              <div className="animate-fadeIn">
                {!adminOtpSent ? (
                  <div className="flex flex-col gap-3">
                    <div>
                      <label className="text-[10px] text-zinc-400 block font-bold mb-1.5 uppercase tracking-wider">Staff Phone Number</label>
                      <input
                        type="text"
                        value={adminPhone}
                        onChange={(e) => {
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      ...
                      <input
                        ...
                        value={adminPhone}
                        disabled
                        className="w-full bg-zinc-950 border border-slate-900 rounded-xl px-3.5 py-2.5 text-xs text-zinc-500 font-semibold outline-none cursor-not-allowed"
                      />
                    </div>

                    <div>
                      <div className="flex justify-between items-center mb-1">
                        <label className="text-[10px] text-zinc-400 block font-bold uppercase tracking-wider">Enter 6-Digit Staff OTP</label>
                        <span className="text-[8px] text-zinc-550 font-bold block">Sent successfully</span>
                      </div>
                      <input
                        type="text"
                        maxLength={6}
                        ...
                        placeholder="••••••"
                        className="w-full bg-zinc-900 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-zinc-200 outline-none focus:border-primary text-center font-mono font-black tracking-widest"
                      />
                    </div>

                    {/* Simulated SMS Gateway Box */}
                    <div className="bg-primary/5 border border-primary/10 rounded-xl p-2.5 text-center">
                      <span className="text-[8px] text-primary/75 font-bold uppercase tracking-wider block mb-0.5">📟 SMS Gateway Simulator</span>
                      <span className="text-[10px] text-zinc-300 font-semibold">Verification Code Sent: <span className="font-mono text-xs font-black text-primary animate-pulse">{adminGeneratedOtp}</span></span>
                      <button
                        onClick={() => { setAdminOtpInput(adminGeneratedOtp); }}
                        className="text-[8px] font-bold text-zinc-400 block mx-auto mt-1 hover:text-primary transition-colors underline"
                      >
                        ⚡ Auto-fill simulated code
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 mt-1">
                      ...
                      <div className="flex justify-between items-center px-1">
                        <button
                          ...
                          className="text-[9px] font-bold text-zinc-500 hover:text-zinc-400 transition-colors uppercase tracking-wider"
                        >
                          ← Edit Number
                        </button>
                        <button
                          onClick={handleAdminSendOtp}
                          disabled={adminOtpCountdown > 0}
                          className={`text-[9px] font-bold uppercase tracking-wider transition-colors ${adminOtpCountdown > 0 ? 'text-zinc-650 cursor-not-allowed' : 'text-primary hover:text-primary/80'
                            }`}
                        >
                          {adminOtpCountdown > 0 ? `Resend OTP in ${adminOtpCountdown}s` : 'Resend OTP Code'}
                        </button>
                      </div>
                    </div>
                  </div>
  ...
  const tabs = [
    ...
    { id: 'chat', label: 'WhatsApp Assistant', icon: MessageSquare },
  ...
  const demoStepsInfo = [
    ...
    { title: '3. WhatsApp Inquiry', desc: 'Navigate to "WhatsApp Assistant" tab and click the prompt "Jubilee Hills haircut at 3:30 PM".' },
