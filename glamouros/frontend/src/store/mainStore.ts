...
interface GlamourStore {
  ...
  checkInAppointment: (bookingId: string) => Promise<any>
...
export const useMainStore = create<GlamourStore>((set, get) => ({
  ...
  login: async (emailOrPhone: string, password: string) => {
    ...
    const searchVal = emailOrPhone.trim().toLowerCase().replace(/\s+/g, '')

    // 1. Check locally registered customers list first!
    if (typeof window !== 'undefined') {
      const existing = localStorage.getItem('glamour_registered_customers')
      const customers = existing ? JSON.parse(existing) : []
      const found = customers.find(
        (c: any) => c.email.toLowerCase() === searchVal || c.phone.replace(/\s+/g, '') === searchVal
      )
      if (found) {
        if (found.password === password) {
          set({ user: found, isAuthenticated: true })
          localStorage.setItem('glamour_user', JSON.stringify(found))
          localStorage.setItem('glamour_token', 'mock_token_' + found.id)
          get().addNotification(`Access Granted: Welcome back, ${found.name}! (Sandbox Mode)`, 'success')
          return { success: true, user: found, token: 'mock_token_' + found.id }
        } else {
          throw new Error('Incorrect password (Sandbox Mode).')
        }
      }
    }
  ...
  registerUser: async (userData: any) => {
    ...
    const mockUser = {
      ...
      password: userData.password, // Keep password for sandbox verification
      role: 'customer',
      created_at: new Date()
    }

    // Save to locally registered customers list
    if (typeof window !== 'undefined') {
      const existing = localStorage.getItem('glamour_registered_customers')
      const customers = existing ? JSON.parse(existing) : []
      const isDuplicate = customers.some(
        (c: any) => c.email.toLowerCase() === userData.email.trim().toLowerCase() || c.phone.replace(/\s+/g, '') === userData.phone.trim().replace(/\s+/g, '')
      )
      if (isDuplicate) {
        throw new Error('Email or Phone is already registered in local sandbox.')
      }
      customers.push(mockUser)
      localStorage.setItem('glamour_registered_customers', JSON.stringify(customers))
    }
  ...
  checkInAppointment: async (bookingId: string) => {
    const { isBackendConnected } = get()
    ...
    if (isBackendConnected) {
      try {
        ...
        if (response.ok) {
          const result = await response.json()
          get().addNotification(`Check-in Confirmed: ${result.message}`, 'success')
          
          // Re-fetch appointments/bookings if needed
          await get().initData()
          return result
        }
      } catch (err) {
        console.error('Failed to check-in via Express API backend', err)
      }
    }

    // Client mock dynamic trigger fallback
    set((state) => {
      ...
      // Credit stylist commission and update notifications
      const appt = state.appointments.find(a => a.id === bookingId)
      if (appt) {
        setTimeout(() => {
          get().addNotification(`Dynamic Check-In QR Authenticated. Customer Checked-In successfully.`, 'success')
        }, 100)
      }
